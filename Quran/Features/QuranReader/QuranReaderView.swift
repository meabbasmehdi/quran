...
struct QuranReaderView: View {
    ...
    private var topBar: some View {
        HStack {
            ...
            HStack(spacing: AppSpacing.md) {
                ...
                .quranFocusStyle(isFocused: focusTarget == .readerQuickSettings, cornerRadius: AppRadius.medium)
            }
        }
        .focusSection()
        .padding(.horizontal, AppSpacing.xl)
    ...
    private func readerContent(ayahs: [Ayah]) -> some View {
        VStack(spacing: 0) {
            if let surah = viewModel.surahInfo {
                ...
                let juzNumber: Int? = {
                    if case .juz(let n) = viewModel.readingSource { return n }
                    return nil
                }()
            ...
            ScrollViewReader { proxy in
                ...
                .onChange(of: audioPlayer.currentAyahNumber) { _, ayahNumber in
                    ...
                    withAnimation(.easeInOut(duration: 0.3)) {
                        proxy.scrollTo(ayahNumber, anchor: .center)
                    }
                }
            }
        }
    ...
    private func savePlaybackState() {
        guard let surah = audioPlayer.currentSurahNumber,
              let ayah = audioPlayer.currentAyahNumber else { return }
    ...
    private func moveFocus(_ direction: MoveCommandDirection) {
        ...
        switch current {
        case .readerAyah(let number):
            guard let index = viewModel.ayahs.firstIndex(where: { $0.numberInSurah == number }) else { return }
            switch direction {
